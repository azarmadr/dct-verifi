...
module dct ( CLK, RST, xin,z_out,dct_2d,rdy_out);
...
reg [10:0] xb0_in,         xb1_in,         xb2_in,         xb3_in;
reg [10:0] xb4_in,         xb5_in,         xb6_in,         xb7_in;
reg [11:0] xb0_reg,        xb1_reg,        xb2_reg,        xb3_reg;
reg [11:0] xb4_reg,        xb5_reg,        xb6_reg,        xb7_reg;
  ...
  if (RST) begin
    ...
    case (indexi)
      0 : begin
        ...
        memory3a    <= 8'd91; memory4a      <= 8'd91;
      end
      1 : begin
        memory1a    <= 8'd126;memory2a      <= 8'd106;
        memory3a    <= 8'd71; memory4a      <= 8'd25;
      end
      2 : begin
      ...
      3 : begin
        ...
        memory4a[7] <= 1'b1;  memory4a[6:0] <= 7'd71; //-8'd71;
      end
      4 : begin
        memory1a    <= 8'd91;
      ...
      5 : begin
        ...
        memory3a    <= 8'd25; memory4a      <= 8'd106;
      end
      6 : begin
        memory1a    <= 8'd49;
        memory2a[7] <= 1'b1;  memory2a[6:0] <= 7'd118;//-8'd118;
        memory3a    <= 8'd118;
        memory4a[7] <= 1'b1;  memory4a[6:0] <= 7'd49; //-8'd49;
      end
      7 : begin
        memory1a    <= 8'd25;
        memory2a[7] <= 1'b1;  memory2a[6:0] <= 7'd71; //-8'd71;
        memory3a    <= 8'd106;
        memory4a[7] <= 1'b1;  memory4a[6:0] <= 7'd126;//-8'd126;
      end
    endcase
  end
end
...
assign p1a_all = addsub1a_comp * memory1a[6:0];/* 8 bits * 7 bits = 15 bits*/
...
always @ (posedge RST or posedge CLK) begin
  if (RST) begin
    ...
    p1a <= (save_sign1a ^ memory1a[7]) ? (-p1a_all) :p1a_all;
    p2a <= (save_sign2a ^ memory2a[7]) ? (-p2a_all) :p2a_all;
    p3a <= (save_sign3a ^ memory3a[7]) ? (-p3a_all) :p3a_all;
    p4a <= (save_sign4a ^ memory4a[7]) ? (-p4a_all) :p4a_all;
    if (indexi == 7)
      indexi <= 0;
    else
      indexi <= indexi + 1;
...
/* After the RAM is enabled, data is written into the RAM1 for 64 clk cycles
Data is written in into each consecutive location
After 64 locations are written into, RAM1 goes into read mode and RAM2 goes into write mode
The cycle then repeats
For either RAM, data is written into each consecutive location
However , data is read in a different order
If data is assumed to be written in each row at a time, in an 8x8 matrix, data is read each column at a time
ie., after the first data is read out, every eight data is read out
Then the 2nd data is read out followed be every 8th.
...
initial begin
  integer itemp;
  for (itemp=0;itemp<64;itemp++)begin
    ram1_mem[itemp] = 11'b0;
    ram2_mem[itemp] = 11'b0;
  end
end

always @ (posedge CLK)
  if (en_ram1reg == 1'b1 && wr_cntr[6] == 1'b0) ram1_mem[wr_cntr[5:0]] <= z_out;


always @ (posedge CLK)
  if (en_ram1reg == 1'b1 && wr_cntr[6] == 1'b1) ram2_mem[wr_cntr[5:0]] <= z_out;
...
always @ (posedge CLK or posedge RST) begin
  if (RST) en_dct2d_reg <= 1'b0;
  else en_dct2d_reg <= en_dct2d;
end
...
always @ (posedge RST or posedge CLK) begin
  if (RST) begin
    addsub3b_comp <= 12'b0; save_sign3b <= 1'b0;
  end
  else begin
    case (add_sub3b[11])
      1'b0: begin
...
always @ (posedge RST or posedge CLK) begin
  if (RST) begin
    addsub4b_comp <= 12'b0; save_sign4b <= 1'b0;
  end
  else begin
    case (add_sub4b[11])
      1'b0: begin
        addsub4b_comp <= add_sub4b[11:0]; save_sign4b <= 1'b0;
      end
      1'b1: begin
        addsub4b_comp <= (-add_sub4b[11:0]) ; save_sign4b <= 1'b1;
      end
    endcase
  end
end
